...
```bash
docker build --platform=linux/amd64 --target <stage> -t <image-name>:<image-version> <build-path>
```
...
## Run
```bash
docker run -d --name api --restart unless-stopped --platform=linux/amd64 --env-file ./.env.production -e APP_NAME=api -p 3010:3010 my-api:1.0.0
```
